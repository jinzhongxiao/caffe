...
class ImageClassifier:
    def __init__(self, model_dir):
        ...
        weights_file = os.path.join(model_dir, 'snapshot_iter_30.caffemodel')
    ...
    def classify(self, fullpath):
        ...
        b = np.zeros((227,227), dtype=image.dtype)    
        g = np.zeros((227,227), dtype=image.dtype)    
        r = np.zeros((227,227), dtype=image.dtype)    

        b[:,:] = self.net.blobs['data'].data[0][0] 
        g[:,:] = self.net.blobs['data'].data[0][1]   
        r[:,:] = self.net.blobs['data'].data[0][2]  


        merged = cv2.merge([b,g,r])  
     
        cv2.imwrite("/home/roger/WorkSpace/caffe/have-fun-with-machine-learning/data/untrained-samples/12.jpg",merged)
        # cv2.waitKey(0)    
        # while True:
        print merged[0][0][:5]
        m = cv2.imread("/home/roger/WorkSpace/caffe/have-fun-with-machine-learning/data/untrained-samples/12.jpg")
        print m[0][0][:5]
